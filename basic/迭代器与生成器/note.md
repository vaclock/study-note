...
生成器函数是一种语法糖，便于简洁创造出迭代器，但是额外支持了可中断执行(`yield`)

生成器函数调用并不会立即执行生成器函数，而是会返回一个迭代器(生成器对象)，要真正执行生成器内部函数代码，必须调用`.next()`方法

`yield`: 暂停执行，并返回一个值给调用者，同时保存当前的执行上下文

`yield *`: 将执行权交给另一个生成器函数
